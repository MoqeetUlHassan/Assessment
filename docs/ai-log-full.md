...
**Adding sites (on request):** any member can add a site from the request form, and sites are strictly bound to the org (the org comes only from the session). The browser run for this feature failed a *different*, previously green check ("edit reason shown"). The cause was a real ordering bug: the page showed "Saved." *before* reloading, so for a moment the success message sat above stale data. The new step had shifted the timing enough to expose it. Fixed in the page (reload, then message, in both the request and admin pages), not by adding a wait to the test.
