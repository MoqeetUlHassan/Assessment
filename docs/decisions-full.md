...
| **Any member of an organization can add a site**, from the request form's "+ Add a new site…" option (`POST /api/sites`) | OrgAdmin-only site management; seed-only sites | User's product decision. The org comes only from the session (the body has no org field, and one sent is ignored); names are unique per org, case-insensitively, with a DB unique index as the race backstop; each creation is audited as `SiteCreated`. Rename and delete aren't built: requests and the spend report reference sites. |
