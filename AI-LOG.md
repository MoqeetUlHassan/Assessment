...
| **Indexing judgment** | Tied every index to a known query, e.g. a partial covering index for the report: `(org, site, completed_at) INCLUDE (actual_cost) WHERE status='Completed'` | No change requested. Keeping the "redundant" status filter was the agent's call, made on evidence | `EXPLAIN` on the SQL EF actually generates: an Index Only Scan with the filter, table reads without it. **A gap only surfaced when I asked whether the requests list was indexed:** the default view still sorted the whole org. Fixed with two indexes matching the list's sort; `EXPLAIN` now shows no Sort step |
| **Migrations** | Generated `InitialSchema` (with the audit trigger added as raw SQL in `Up`/`Down`) and later `RequestListIndexes`; migrate-on-startup in Development only | Approved schema changes before the migration was generated | Read the generated SQL line by line; `has-pending-model-changes` after later model edits. It caught that the snake_case rename also renames EF's own history table, breaking older databases |
